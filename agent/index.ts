...
- Search and discuss matching jobs directly in chat using the web_search tool, applying the saved skills and location.
...
const agent = new Agent({
  ...
  tools: {
    ...tools,
    // Anthropic's server-side web search, billed against ANTHROPIC_API_KEY (no
    // jobs-API key). Used for job discovery in chat and scheduled digests.
    //
    // web_fetch is intentionally NOT enabled: @ai-sdk/anthropic v4 cannot
    // serialize its error results (web_fetch_tool_result_error, e.g. a blocked
    // URL) back into a prompt, so a single failed fetch poisons the whole
    // conversation (convertToAnthropicPrompt throws on every later turn). Link
    // liveness is verified deterministically in code instead — see
    // agent/scheduler.ts -> isLivePosting.
